...
router.post('/upload', async (ctx, next) => {
    ...
    if (!xfile) {
        ctx.body = ResultUtils.errorMsg("无法匹配name='xfile'的文件！");
    } else {
        // 上传单个文件
        try {
...
router.post('/uploadBase64Img', async (ctx, next) => {
    ...
    if (!base64Img) {
        ctx.body = ResultUtils.errorMsg("无法匹配name='base64Img'的数据");
    }
    // 上传单个文件
    try {
...
router.post('/resolveCode', async function downloadFile(ctx, next) {
    try {
        ...
        ctx.body = ResultUtils.successData(text);
    } catch (e) {
        ctx.body = ResultUtils.errorMsg('解析验证码异常，' + e.toString());
    }
});
